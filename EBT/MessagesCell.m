...
-(void) initCell {
    ...
    _detailText.frame = CGRectMake(90, 40, kScreenBounds.size.width - 140, 38);
...
-(void) fillCell:(NSDictionary*) item forRow:(int) row {
    ...
    float height = [Utils heightWithText:item[kContent] andFont:_detailText.font andMaxWidth:_detailText.frame.size.width andMaxHeight:50];
    float top = (88 - height - 25)/2;
    _text.frame = CGRectMake(_text.frame.origin.x, top, _text.frame.size.width, _text.frame.size.height);
    _detailText.frame = CGRectMake(_detailText.frame.origin.x, top+25, kScreenBounds.size.width-120, height);
